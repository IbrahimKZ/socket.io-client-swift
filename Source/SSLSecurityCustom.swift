//
//  SSLSecurityCustom.swift
//  Socket.IO-Client-Swift
//
//  Created by Ibrakhim Nikishin on 13.01.2018.
//
...
open class SSLCertCustom : NSObject {
    var certData: Data?
    var key: SecKey?
...
open class SSLSecurityCustom : NSObject, SSLTrustValidator {
    ...
    public convenience init(usePublicKeys: Bool = false) {
        ...
        let certs = paths.reduce([SSLCertCustom]()) { (certs: [SSLCertCustom], path: String) -> [SSLCertCustom] in
            var certs = certs
            if let data = NSData(contentsOfFile: path) {
                certs.append(SSLCertCustom(data: data as Data))
            }
            return certs
        }
    ...
    /**
     ...
     - returns: a representation security object to be used with
     */
    public init(certs: [SSLCertCustom], usePublicKeys: Bool) {
        ...
        if self.usePublicKeys {
            DispatchQueue.global(qos: .default).async {
                let pubKeys = certs.reduce([SecKey]()) { (pubKeys: [SecKey], cert: SSLCertCustom) -> [SecKey] in
                    var pubKeys = pubKeys
            ...
            let certificates = certs.reduce([Data]()) { (certificates: [Data], cert: SSLCertCustom) -> [Data] in
